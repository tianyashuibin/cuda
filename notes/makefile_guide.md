...
```
目标: 依赖
	命令（必须以一个 Tab 字符开头，绝对不能用空格，否则会报 missing separator 错误）
```
...
| `$@` | **目标文件名** | 生成的文件路径 |
| `$<` | **第一个依赖文件** | 通常用于编译单个源文件 |
| `$^` | **所有依赖文件** | 链接阶段，把所有 .o 文件连起来 |
| `$?` | **比目标新的依赖** | 只处理那些被修改过的文件 |
| `$+` | **所有依赖（含重复）** | 较少用，不进行去重处理 |
...
*   `:=` (**立即求值**)：在定义这一行就固定了值，不会受后面变量变化的影响（最推荐，性能好）。
*   `=` (**延迟/递归求值**)：在使用时才去计算。哪怕引用的变量在后面定义也没关系。
*   `:` (**规则定义**)：用于分隔目标和依赖（例如 `all: build`），它不是赋值。

### 2. .PHONY (伪目标)

通常定义在文件末尾：`.PHONY: all clean`

*   **作用**：告诉 `make` 这些是“指令”而不是“文件”。
*   **为什么要用**：如果你的目录下刚好有一个叫 `clean` 的文件，没有声明 `.PHONY` 的话，执行 `make clean` 就会提示“目标已是最新”，而不会跑删除命令。声明后则会无视文件，强制执行。
